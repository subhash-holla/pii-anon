...
class TestCrossTypeArbitration:
    def test_org_span_is_not_also_a_person(self) -> None:
        ...
        assert "Bluth Company" in _spans(text, "ORGANIZATION")
        assert "Bluth Company" not in _spans(text, "PERSON_NAME")
    ...
    def test_job_title_span_is_not_also_a_person(self) -> None:
        ...
        assert "Medical Director" in _spans(text, "JOB_TITLE")
        assert "Medical Director" not in _spans(text, "PERSON_NAME")
    ...
    def test_health_condition_span_is_not_also_a_person(self) -> None:
        ...
        assert "Acute Bronchitis" in _spans(text, "HEALTH_CONDITION")
        assert "Acute Bronchitis" not in _spans(text, "PERSON_NAME")
    ...
    def test_medication_span_is_not_also_a_person(self) -> None:
        ...
        assert "Albuterol Inhaler" in _spans(text, "MEDICATION_NAME")
        assert "Albuterol Inhaler" not in _spans(text, "PERSON_NAME")
