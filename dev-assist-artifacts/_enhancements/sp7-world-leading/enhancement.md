...
| — | STEP 3 Phase B fusion recalibration (config-first, swarm) — the swarm inherits ALL Phase A gains via FloorProjectingFusion (swarm ≥ vanilla); sp6 already populated the per-type acceptance map; per-(engine,type) TRAIN-derived refinement PENDING | IN PROGRESS |
