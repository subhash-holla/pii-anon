...
| B✓ | **★ Phase B COMPLETE — per-(engine,type) swarm acceptance (panel #9/#4, user-authorized, `f4b1c32`).** `SwarmConfig.single_engine_min_confidence_by_engine` (engine→{type→bar}), checked before the per-type fallback, fail-loud validated. Opens the GLiNER semantic single-engine channel the per-type bars left inert (GLiNER conf caps ~0.87, so ORG@0.90 was structurally unreachable). Default GLiNER bars ORG 0.82 / ADDR 0.80 / LOC 0.80 / DATE_TIME 0.82 — all ≤ the per-type fallback ⇒ STRICTLY ADDITIVE ⇒ leak-safe. **Home swarm strict F2 0.8989→0.8996 (+0.0007, home IMPROVES); nemotron swarm relaxed F2 0.3830→0.3914 (+0.0084, P&R both up).** Close CLOSE_PASS (0 coverage-shrink). **SDO re-cert VERIFIED: regenerated canonical run → re-ran `competitive_supremacy` → J 0.2775→0.28, all G PASS, verdict NOT_YET (unchanged). Ships as DEFAULT.** The regenerated artifact was NOT committed (it carries the scope=data-v2.0.0-vs-version-2.2.0 provenance bug the PARALLEL session owns; committing would race their fix) — the artifact regen+commit belongs to that provenance workflow. TRAIN download NOT needed (calibration from the measured GLiNER confidence ranges + the home-floor gate). Was: |
...
## Autonomous continuation (2026-07-13, panel roadmap remainder)

| # | Delta | Status |
|---|---|---|
| ML | **Multilingual hygiene pair (`1ec6925`):** `_drop_ssn_shadowed_national_id` (same-span dedup, leak-safe, all paths) + `_ORGANIZATION_CONTEXT` honorific veto. **Home overall strict F2 0.9033→0.9062 (+0.0029); ar +0.032, zh +0.031, hi +0.026, ko +0.024, ja +0.007.** Close: only honorific tokens (non-PII by spec) lose coverage | DONE (TDD + close) |
| NUM | **Numeric-field PII leak fix (`f758e0f`):** `{'ssn_number': 573337773}` passed through UNMASKED — `_scannable_text` coerces int/float (not bool) for detection; numeric fields with findings are masked; benign numerics keep value+type. Strictly additive | DONE (TDD + close) |
| PHN | **Dotted-NANP phone validator fix (`cbba16c`):** the version-number FP rule rejected every dotted phone (765.340.8856). 3-3-4 shape exempted; NANP rules still apply. Home neutral; real-world recall win | DONE (TDD + gate) |
| HRD | **run_file error surfacing + leak-branch coverage (`0e05fea`):** typed error entries + per-failure/summary warnings + `on_error="raise"`; tests for the `_COMMON_GIVEN_NAMES` override and every eval-drop EFFECT branch; text_all dict-rebuild dedup | DONE (TDD) |
...
change got the mandatory adversarial close; detection byte-identity holds throughout
(the SDO producer `canonical_run.py` is `detect_only`, never the transform path).

| # | Delta | Status |
|---|---|---|
| NANP | **International-phone posture (`1a3bf48`):** positional NANP rules (area/exchange N-digit, invalid-area-code table) gated to WEAKLY-formatted candidates via accept-or-fall-through; `+CC`, `00`-intl, trunk-`0` grouped, and strong-format E.164 shapes pass through. Home delta 0 (strong-format branch keeps the `_INVALID_US_AREA_CODES` pin). Real-world non-US recall win | DONE (TDD + close) |
| UNI | **Unicode-normalization detection pre-pass (`2af02f0`):** `normalize_for_detection` strips Cf format chars (ZWSP/ZWNJ/ZWJ/BOM/soft-hyphen) + per-char NFKC folds fullwidth/compat forms for DETECTION, remapping spans to ORIGINAL offsets (so the mask covers interior obfuscation chars). ASCII fast-path ⇒ home byte-identical; original text never mutated. ADDITIVE (patterns match text they couldn't before; spans only expand) ⇒ leak-safe. Closes the zero-width/fullwidth evasion class | DONE (TDD + close) |
| DENY | **Deny-list canonical folding (`583cdfd`):** `canonical_deny_form` (lowercase + whitespace-collapse + edge-punct strip) applied symmetrically at load and lookup so `"John Doe"`, `" john  doe "`, `"John Doe."` all match one entry. Does NOT fold unicode confusables/zero-width (deny-list is a name→drop suppressor; broadening it only DROPS more, so kept conservative). Home neutral | DONE (TDD + close) |
| PROV | **Honest provenance scope (`eaaaa11`):** the canonical producer stamped `scope=data-v2.0.0` while the substrate was v2.2.0 (a fabrication-vector red). Resolver + validator now stamp/accept `data-v<resolved-version>`; restored the `artifacts/benchmarks/*.json` the docs-link test reads. Cleared BOTH standing suite reds. Detection numbers unchanged (scope label only) | DONE (both reds green) |
| CORE | **★ Production value-consistent (coreference) masking — the report masker's runtime twin (`transforms/value_consistency.py` + orchestrator pass-2).** `TransformConfig.value_consistent_masking` (default ON) → after the primary transform, sweep the ORIGINAL field text (never the transformed text — an HMAC token can contain a short surface) for every REMAINING verbatim occurrence of a detected sticky-type value (13 types, len≥4, cross-field) and redact it with the SAME replacement. Only fields with an ACTUAL residual are re-assembled ⇒ no-coreference payloads byte-identical (link_audit included). Shares `VALUE_CONSISTENT_TYPES`/`_MIN_LEN` with `reconstruction_resistance_cli` so report-masker and runtime stay in lock-step. **Adversarial close: 600 probes → 0 detection-inversions, 0 value-reintroductions, 0 coverage-reductions; `coverage(on) ⊇ coverage(off)` by construction (merged pass-1∪residual spans over one original text); linear `str.find` ⇒ no ReDoS.** home_risk=none (transform-only; SDO/home strict-F2 untouched by construction). **The full-suite close caught a metric-inflation bug:** the pass-2 audit entries carried alias `mention_text` but no `cluster_id`, so `test_continuity_benchmark`'s `long_context_alias_precision = mentions_linked / primary_cluster_mentions` exceeded 1.0 (1.486) — numerator grew, denominator didn't. Fix: thread the source value's `cluster_id` through `build_surfaces`→`sweep`→audit so a residual of "Jack" is attributed to Jack's identity cluster (audit trail MORE correct; both terms grow together ⇒ metric ≤ 1.0). | DONE (TDD 17 + close) |
