...
| Story | S7-04 |
| Sprint | 7 |
| State | **DONE** (2026-06-09; SO-16. 5/5-APPROVE story gate (0 MAJOR; 4 MINOR remediated in-loop) THEN the MANDATORY adversarial close ran **2 ROUNDS**: round-1 `wf_2e3f36d5-afb` caught + independently-verified a **MAJOR breach-bury fabrication** (a real latency/audit breach in the PRESENT half was buried into PENDING by omitting the OTHER block, escalating NOT_YET→PROVISIONAL_SOTA); fixed RED→GREEN; round-2 confirmatory `wf_4c4df480-634` = **CLOSE_PASS, 0 upheld / 764 probes**, both fabrication refuters + the g5-contract refuter completed, null_refuters=[], off-limits byte-identical. See §Evidence.) |
| Size | M |
...
**Commits (RED→GREEN per part, on `pdlc/sota-program`):** 689f387/793f999 (ceiling registry), 5f975df/f38b687 (gate `_g5_audit_latency` + `_safe_repr`/`_safe_names` DoV), 5ae81b7/4e6cf0b (producer G5 emission + validate step-7), d5be194 (import-boundary scan +`latency_ceilings.py`), 53ab984 (contention recalibration 250/500/1000→500/1000/2000 + min-of-3 estimator), 2f7215c (story-gate MINOR remediation), then the close MAJOR fix **RED `<breach-bury RED>` → GREEN `3b842e8`** (`_g5_audit_latency` evaluates each present half before deciding missing-shape).

**Final control-path md5:** gate `competitive_supremacy.py` = `3b842e81c3f03eafd11f9c655c1789a0`; producer `canonical_run.py` = `d8f0f80e113c3b5d59c06d0b5fd36fac`; registry `latency_ceilings.py` = `848f21bef3731ad56bae3b0b5aa86fee`. **Off-limits byte-identical:** `competitor_compare.py` `7cae16c8…`, `competitor_tiers.py` `d9202479…`. **User-WIP byte-identical:** orchestrator `0afc6dee…`, test_moe `910e9cd6…`, README `b5cb977b…`, benchmark-diagnostics `47f9b116…`.

**Story gate (5/5 APPROVE; `_reviews/story/S7-04/`):** security-sast (PRIMARY) + axiom-compliance + traceability + requirements-coverage + code-quality. 0 MAJOR. 4 MINOR remediated in-loop: 2 stale test docstrings (G5-PENDING→PASS; LONE→TWO determinism exclusions), the `honest_axis` G5 entry, the story FR-025/FR-028/path/n_records notes, and the stale NFR-009 verification-matrix row.

**MANDATORY adversarial close (2 rounds):**
- **Round-1 `wf_2e3f36d5-afb` = CLOSE_FAIL** — 1 confirmed MAJOR (the **breach-bury fabrication**): `_g5_audit_latency` short-circuited to PENDING the moment ONE G5 block was absent, *without evaluating the present block* — so a real breach in the present half (over-budget latency / leaked span) was laundered into PENDING, escalating the verdict NOT_YET→PROVISIONAL_SOTA (independently reproduced via both `from_artifacts` and the shipped CLI; both directions latency↔audit). The breach-outranks-missing rule only fired when *both* blocks were present dicts. 6 refuters / 370 probes; 0 crashes / 0 other fabrications; off-limits byte-identical.
- **Fix (RED→GREEN `3b842e8`):** an absent/non-dict block becomes a missing-shape `_G5Half(ok=None)` instead of a short-circuit, so the breach-outranks-missing combiner applies at the whole-block level. Surgical: a clean-present + absent-other still reads PENDING (a single half cannot certify); the both-absent path keeps its successor-named detail (honest-surface stability); smoke verdict byte-identical.
- **Round-2 confirmatory `wf_4c4df480-634` = CLOSE_PASS — 0 upheld / 764 probes** (231+253+72+62+62+84), both fabrication refuters (flat-scalar + nested-value) + the g5-contract-boundary refuter completed, null_refuters=[], off-limits byte-identical.

**SDO verdicts (honest, byte-stable vs SO-15):**
- *Committed smoke artifact* (`pii-anon supremacy` default): **NOT_YET**, binding `canonical_claim_run=False (G7)`, J=1.0 mle-bootstrap; G3·G6 PASS, G1·G2·G4·**G5**·G7 the rest (G5 honestly PENDING — no G5 fields on the smoke artifact). UNCHANGED.
- *Freshly produced certified artifact* (`produce_canonical_artifact(max_samples=8)`): **NOT_YET**, `canonical_claim_run=True`, binding **G6 FAIL** (core F2 0.7214 < best Tier-R gliner 0.75; coverage 0.824). **G1·G2·G3·G4·G5·G7 ALL PASS — ZERO pending axes** (G5 now computes a REAL PASS: measured swarm p50/p95/p99 within the committed 500/1000/2000 ms ensemble ceiling + a clean 4-channel audit). J=0.0. **G5 PASS did NOT flip the verdict — G6 binds (decision A — not a regression). This is COMPLETENESS, not a flip, exactly as scoped.**

**Quality:** full xdist suite (`PYTHONPATH=src .venv/bin/python -m pytest -n auto`) exit 0, **3685 passed / 16 skipped / 0 failed**, coverage **88.66%** (≥84 gate). ruff clean (src+tests); mypy clean under BOTH `mypy src/pii_anon` AND `--strict` (137 files). The 16 skips are env-gated (numpyro/jax/dataset/pyarrow) — none in the S7-04 files.

**DoD:** all checkboxes met. The R10 per-detector-class p50/p95/p99 refinement is the `detector_class` Pass-2 seam; full-census latency + live-agent audit are tracked Pass-2.
