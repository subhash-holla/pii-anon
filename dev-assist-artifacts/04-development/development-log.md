...
- **Next: S3-03** (bayes-bt NumPyro NUTS = claim-grade default; hard NFR-001 convergence gate R̂≤1.01 ∧ ESS≥400/param ∧ 0 divergences; `bayes-eval` optional extra) → **S3-04** (coherent significance BY CONSTRUCTION + record-level paired outcomes from `per_record_f1` + Davidson ties — must pin S3-02's engine-only `fit_paired`/`paired_bootstrap` shape, the cross-reviewer joint signal). Then the **SDO `CompetitiveSupremacyGate`** (J needs S3-03's posterior), the security MUSTs (S2-05 / S5-04 / S6-03), and S2/S4/S5/S6/S7.
