...
class ConfidenceConfig(BaseModel):
    ...
    context_boost: float = Field(
        default=0.10, description=("Confidence increase when context keywords are found near the match.")
    )
    context_penalty: float = Field(
        default=0.15,
        description=("Confidence decrease for high-false-positive entity types when context keywords are absent."),
    )
    context_window: int = Field(
        default=50, description=("Number of characters (±) around the matched span to search for context keywords.")
    )
    confidence_cap: float = Field(default=0.99, description=("Maximum confidence score after context boosting."))
    confidence_floor: float = Field(default=0.40, description=("Minimum confidence score after context penalty."))
...
class FusionConfig(BaseModel):
    ...
    iou_threshold: float = Field(
        default=0.5,
        description=("Minimum Intersection-over-Union for two entity spans to be considered overlapping and merged."),
    )
    min_gap_chars: int = Field(
        default=5,
        description=("Character gap after which span overlap checking is skipped (performance optimization)."),
    )
...
class MoEConfig(BaseModel):
    ...
    top_k: int = Field(default=3, description=("Number of experts to activate per entity type."))
    performance_floor: bool = Field(
        default=True,
        description=(
    ...
    min_expert_weight: float = Field(default=0.15, description=("Minimum weight for the performance floor expert."))
    iou_threshold: float = Field(default=0.5, description=("IoU threshold for span overlap clustering."))
    custom_experts: dict[str, dict[str, Any]] = Field(
        default_factory=dict, description=("User-defined expert registrations.")
    )
    calibration_path: str | None = Field(
        default=None, description=("Path to offline calibration JSON. None = auto-detect ~/.pii_anon/calibration.json.")
    )
    online_calibration: bool = Field(
        default=False, description=("Enable online calibration (EMA weight updates during a session).")
    )
    online_calibration_alpha: float = Field(default=0.05, description=("EMA alpha for online calibration."))
    calibration_min_samples: int = Field(
        default=10, description=("Minimum labeled examples per (engine, entity_type) to accept offline calibration.")
    )
    similarity_threshold: float = Field(
        default=0.95, description=("Cosine similarity threshold for duplicate expert detection.")
    )
    similarity_action: Literal["warn", "reject"] = Field(
        default="warn", description=("Action when similarity threshold is exceeded.")
    )
...
class RiskConfig(BaseModel):
    ...
    low_risk_threshold: float = Field(
        default=0.90, description=("Confidence scores >= this are classified as 'low' risk.")
    )
...
class RouterConfig(BaseModel):
    ...
    ensemble_confidence_threshold: float = Field(
        default=0.70,
        description=("Low-confidence threshold for ensemble objective. Below this triggers escalation (if enabled)."),
    )
    accuracy_confidence_threshold: float = Field(
        default=0.88, description=("Low-confidence threshold for accuracy objective.")
    )
    balanced_confidence_threshold: float = Field(
        default=0.80, description=("Low-confidence threshold for balanced objective.")
    )
    ensemble_concurrency_cap: int = Field(
        default=8, description=("Maximum number of engines to run concurrently in ensemble mode.")
    )
    accuracy_concurrency_cap: int = Field(default=4, description=("Maximum concurrent engines for accuracy objective."))
    balanced_concurrency_cap: int = Field(default=3, description=("Maximum concurrent engines for balanced objective."))
    segmentation_token_threshold: int = Field(
        default=2000,
        description=("Minimum document token count to enable segmentation for long documents and multilingual mixes."),
    )
...
class CompetitorPolicyConfig(BaseModel):
    ...
    allowed_adapters: list[str] = Field(default_factory=lambda: ["spacy-ner-compatible", "stanza-ner-compatible"])
